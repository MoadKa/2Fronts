...
- **A concierge is now switched off even when Stripe marks the subscription "unpaid" instead of cancelling it.** Access removal previously depended on Stripe firing a "subscription deleted" event after failed retries. If the Stripe dunning setting were ever changed to "mark unpaid" (or a subscription were cancelled without deletion), the concierge would have kept running for free forever. The billing webhook now also deactivates on a subscription reaching `unpaid` or `canceled`, and re-activates the concierge if that subscription later recovers to good standing (a paid-off `unpaid` invoice), so a customer who catches up is never locked out. Access still stays live during the retry grace window (`past_due` is untouched).
