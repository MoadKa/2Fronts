...
**Parallelization (`superpowers:dispatching-parallel-agents`):** Tasks 1→2 are strictly sequential (foundation). Once Task 2 merges, dispatch Tasks 3 and 6 to two parallel subagents in separate worktrees (no shared files: catalog pages vs. admin catalog pages touch different files, both only add to the shared `AutomationService`— see each task's Interfaces). Task 4 starts only after Task 3 merges; Task 5 after Task 4; Task 7 needs both Task 4 and Task 6 merged, so it queues behind whichever finishes last. Never run two tasks in parallel that modify the same file (e.g. don't parallelize Task 4 and Task 7 — both touch `RequestService`).
